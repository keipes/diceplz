  ...
  return (
    <div ref={elementRef} style={{ minHeight: '400px' }}>
      ...
      {(isVisible || hasBeenVisible) ? (
        <div style={{ opacity: isVisible ? 1 : 0.3 }}>
          {children}
        </div>
      ) : (
        <div 
  ...
  return (
    <>
      {childArray.map((child, index) => (
        <div key={index} data-chart-index={index} style={{ minHeight: '400px' }}>
          {visibleCharts.has(index) ? (
            ...
            >
              <span style={{ opacity: 0.6 }}>Chart {index + 1} (Performance Limited)</span>
            </div>
          )}
        </div>
      ))}
