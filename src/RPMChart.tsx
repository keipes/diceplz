    ...
    for (const [weaponName, stats] of weaponData) {
        ...
        if (showBurst) {
            if (stats.rpmBurst) {
                ...
                burstBackgroundColors.push('hsl(' + StringHue(weaponName) + ', 50%, 50%)');
            } else {
                burstData.push(null);
                burstBackgroundColors.push('hsl(' + StringHue(weaponName) + ', 50%, 50%)');
            }
        ...
        if (showSingle) {
            if (stats.rpmSingle) {
                ...
                singleBackgroundColors.push('hsl(' + StringHue(weaponName) + ', 50%, 50%)');
            } else {
    ...
    return (
        <div>
            ...
            <div className="button-container">
                <button className={autoClass} onClick={_ => setShowAuto(!showAuto)}>Auto</button>
                <button className={burstClass} onClick={_ => setShowBurst(!showBurst)}>Burst</button>
                <button className={singleClass} onClick={_ => setShowSingle(!showSingle)}>Single</button>
            </div>
